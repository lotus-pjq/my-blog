...
## 题目：- # P12786 [ICPC 2024 Yokohama R] Greatest of the Greatest Common Divisors
## 题目背景
...
第三行包含一个正整数 $q$，指定要考虑的序列区间数量，其值不超过 $10^5$。  随后是 $q$ 行，每行指定序列中一个要考虑的区间。其中第 $i$ 行包含两个整数 $l_i$ 和 $r_i$ $(1 \leq l_i 
using namespace std;
...
int l=1,r=0;
cnt[1]=1;
for(int i=1;ip[i].l) add(a[--l]);
while(rp[i].r) del(a[r--]);
ans[p[i].id]=mx;
}
