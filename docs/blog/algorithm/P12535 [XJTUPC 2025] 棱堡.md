---
title: P12535 [XJTUPC 2025] 棱堡
date: 2026-03-09
category: 题解
tags:
  - 算法
description: P12535 [XJTUPC 2025] 棱堡相关的算法笔记和代码模板
---
...
- 题目:
	...
	- ## 输入输出样例 #[[P3265 [JLOI2015] 装备购买]]
	- ### 输入 #[[P3265 [JLOI2015] 装备购买]]
...
- 代码：
	- ```C++
	  ...
	  void Andrew(){//凸包（Andrew算法）
	      ...
	      n=top-1;
	      //for(int i=1;i<=n;i++) cout<<s[i].id<<" "<<s[i].x<<" "<<s[i].y<<endl;
	  }
	  void solve(){
	      cin>>n;n2=n;
	      ...
	      bool f=true;
	      for(int i=1;i<=n;i++)
	          if(abs(s[i].id-s[i%n+1].id)==1||abs(s[i].id-s[i%n+1].id)==n2-1)
	              {f=false;break;}
	      if(f) cout<<"YES"<<endl;
	      else cout<<"NO"<<endl;
	  }
