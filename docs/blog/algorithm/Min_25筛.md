...
tags:
  - 算法
description: Min_25筛相关的算法笔记和代码模板
---
	...
	- ```C++
	  ...
	  void EulerSieve(){
	  	...
	  	for(int i=1;i<=cnt;i++){
	  		...
	  		//sum[i]=(sum[i-1]+f(p[i]) )%mod;
	  		sum2[i]=(sum2[i-1]+p[i]*p[i]%mod)%mod;
	  	}
	  }
	  int getid(int x){return x<=sq?id1[x]:id2[n/x];}
	  int f1(int x){x%=mod;return x*(x+1)/2%mod;}
	  int f2(int x){x%=mod;return x*(x+1)%mod*(2*x%mod+1)%mod*inv6%mod;}
	  int S(int x,int j){
	  	if(p[j]>x) return 0;
	  ...
	  signed main(){
	  	...
	  	for(int l=1,r;l<=n;l=r+1){
	  		...
	  		//g[m]=∑_{i:2->w[m]}_f'(i) (将所有数视为质数)
	  		if(w[m]<=sq) id1[w[m]]=m;
	  		else id2[n/w[m]]=m;
	  	}
	  	for(int i=1;i<=cnt;i++){
	  		for(int j=1;j<=m&&p[i]*p[i]<=w[j];j++){
	  			g1[j]=(g1[j]+mod-p[i]*(g1[getid(w[j]/p[i])]-sum1[i-1])%mod)%mod;
...
- ## 板子2(小于等于n的素数个数):
	- ```C++
	  ...
	  void EulerSieve(){
	  	...
	  	for(int i=1;i<=cnt;i++) sum[i]=i;
	  
	  }
	  int getid(int x){return x<=sq?id1[x]:id2[n/x];}
	  int S(int x,int j){
	  	if(x<2) return 0;
	  	if(p[j]>x||j>cnt) return 0;
